...
while IFS=$'\t' read -r plugin_package plugin_version plugin_filename plugin_url plugin_sha256; do
  ...
  printf '%s  %s\n' "$plugin_sha256" "$plugin_archive" | shasum -a 256 -c -
...
"$NODE_EXE" "$PROJECT_ROOT/scripts/patch-historical-descriptor.mjs" "$STAGE/app"
...
"$NODE_EXE" "$PROJECT_ROOT/scripts/verify-default-plugin-peers.mjs" "$STAGE"
...
"$NODE_EXE" "$PROJECT_ROOT/scripts/verify-historical-session.mjs" "$STAGE/app" "$COMPONENT_LOCK_FILE"
