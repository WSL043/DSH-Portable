...
let fetchStage = 'list-successful-runs'
try {
  ...
  // This report is advisory. A transient GitHub API/artifact outage must not
  // invalidate a finished product build; retain the current footprint and
  // make the missing comparison explicit in the job summary.
  if (error?.code === undefined && error?.signal === undefined) throw error
  result = { status: 'baseline-fetch-failed', platform: current.platform,
    stage: fetchStage, reason: `Previous build could not be fetched (${error.code ?? error.signal}).` }
}
