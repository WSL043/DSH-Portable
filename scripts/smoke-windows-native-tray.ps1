...
function Test-NativeTaskNotification {
    ...
    $Shown = $NativeNotificationType.GetMethod('Show', $AllFields).Invoke($null, [object[]]@($Session, $true))
    if (-not $Shown) { throw 'Native task notification could not be sent to Windows Action Center' }
    ...
    # desktop capture. CI still exercises the same native Show call even when
    # it does not retain screenshots.
    if ($CaptureDirectory) {
