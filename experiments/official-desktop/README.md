...
`launch-hidden-windows.ps1` runs a disposable executable on a private Windows desktop, starts it suspended, assigns it to a job, and terminates that job after a bounded interval. It requires `DSH_PORTABLE_DEVELOPMENT_ROOT`, `APPDATA`, and `LOCALAPPDATA` to point inside one isolated root. Optional `-CloseAfterMilliseconds` posts WM_CLOSE only to visible windows owned by the launched process on that private desktop; JSON output distinguishes natural exit from timeout cleanup. Closing the main workspace may hide it to the tray, so this is not automatically an application-exit test.

Use a unique local CDP port with `probe-welcome.mjs`: `readiness|set-marker|get-marker|enter-workspace` targets the exact checkout Welcome URL; `workspace-readiness|open-plugins|new-chat` targets `dsh-app://app/`. `enter-workspace` invokes the official skip-configuration action; plugin/new-chat modes use visible official buttons. These are synthetic environment probes, not general browser automation. They require `DSH_OFFICIAL_DESKTOP_APP_ROOT`; never use an existing user profile. Readiness output reports visible editors, not successful typing or inference.
