...
test('Windows GUI is a native WebView2 host with its own stable taskbar identity', async () => {
  ...
  assert.match(navigationFlow, /WaitForWorkspaceFirstPaintAsync\(url\)/)
  assert.match(navigationFlow, /RevealDesktopSurface\(\)/)
  assert.match(navigationFlow, /Navigate\(url\)/)
  assert.doesNotMatch(navigationFlow.slice(0, navigationFlow.indexOf('Navigate(url)')), /Opacity\s*=\s*1/)
  assert.match(navigationFlow, /private void RevealDesktopSurface\(\)[\s\S]+launchPanel\.Visible\s*=\s*false;[\s\S]+webView\.BringToFront\(\);[\s\S]+Opacity\s*=\s*1/)
  assert.match(host, /else if \(desktopStart\) Opacity = 0\.01/)
