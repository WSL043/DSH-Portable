...
test('Windows GUI is a native WebView2 host with its own stable taskbar identity', async () => {
  ...
  assert.match(host, /DSH_PORTABLE_STARTUP_STARTED_AT/)
  assert.match(host, /AppendStartupTrace\("native-host", "process-start"/)
  assert.match(host, /AppendStartupTrace\("native-host", "portable-cli-begin"/)
  assert.match(host, /AppendStartupTrace\("native-host", "portable-cli-complete"/)
  assert.match(host, /AppendStartupTrace\("native-host", "interactive-ready"/)
...
test('Windows startup reveals only the full-size official DSH boot surface', async () => {
  ...
  assert.match(host, /productLabel\.Text\s*=\s*"DeepSeek Harness"/)
  ...
  assert.doesNotMatch(host, /RevealDesktopBootSurface|NavigateToString/)
  assert.match(host, /native-loading-ready/)
  ...
  assert.doesNotMatch(firstPaintProbe, /MutationObserver|lastMutation/)
  assert.match(firstPaintProbe, /readyPolls>=3/)
  assert.match(firstPaintProbe, /visibleControls>=2/)
...
test('CI release gate verifies native desktop ownership, lifecycle, and application identity', async () => {
  ...
  assert.match(dataExportSmoke, /fixtureSessionId = 'session-[0-9a-f-]+'/)
  ...
  assert.match(dataExportSmoke, /projectKey\(path\.join\(root, 'workspace'\)\)/)
  assert.match(dataExportSmoke, /version: 0/)
  assert.doesNotMatch(dataExportSmoke, /isSeeded: false/, 'v0 physical headers omit seedLength for unseeded sessions; isSeeded is a logical-only field')
  ...
  assert.match(dataExportSmoke, /document\.querySelectorAll\('\[role="treeitem"\]\[aria-expanded\]'\)/)
  assert.match(dataExportSmoke, /07-restored-workspace-list-failure\.png/)
  assert.match(dataExportSmoke, /07-restored-session-list-failure\.png/)
  assert.match(dataExportSmoke, /bodyText: String\(document\.body\?\.innerText \|\| ''\)/)
  assert.match(dataExportSmoke, /const title = 'Portable migration proof'/)
  assert.doesNotMatch(dataExportSmoke, /session_projcache\.json/, 'migration acceptance must not fabricate upstream derived caches')
  assert.match(dataExportSmoke, /item\.click\(\)/)
  ...
  assert.match(workflow, /\$MigrationNode = Join-Path \$MigrationProduct 'runtime\\node\\node\.exe'/)
  assert.match(workflow, /& \$MigrationNode scripts\/smoke-windows-data-export\.mjs \$MigrationProduct \$MigrationEvidence/)
