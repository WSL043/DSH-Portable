...
test('Windows startup is one full-size product shell with the same activity language as DSH boot', async () => {
  ...
  assert.match(preNavigate, /launchPanel\.Visible\s*=\s*true/)
  assert.match(preNavigate, /launchPanel\.BringToFront\(\)/)
  assert.doesNotMatch(preNavigate, /launchPanel\.Visible\s*=\s*false/)
  const firstPaintProbe = host.slice(host.indexOf('private async Task<bool> WaitForWorkspaceFirstPaintAsync'), host.indexOf('private async Task<bool> ProbeWorkspaceDomAsync'))
  assert.match(firstPaintProbe, /\[data-dsh-boot\]/)
  assert.match(firstPaintProbe, /bootVisible/)
