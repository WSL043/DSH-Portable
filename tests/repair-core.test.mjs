...
test('support report is bounded, useful, and removes credentials and conversation content', async (t) => {
  ...
  await writeFile(path.join(layout.logsDir, 'data-import-latest.jsonl'), '{"operation":"data-import","phase":"rollback-complete","password":"import-secret"}\n')
  ...
  assert.match(source, /data-import-latest\.jsonl/)
  assert.match(source, /rollback-complete/)
  assert.match(source, /runtimeProcesses/)
  assert.doesNotMatch(source, /secret-token|sk-private|portable-control-secret|workspace-auth-secret|workspace-secret|startup-secret|import-secret|private conversation/)
