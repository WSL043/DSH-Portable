...
test('update guidance describes the component update path without exposing internals', async () => {
  ...
  assert.match(chinese, /替换内核前会先用新内核检查现有 Profile 和插件能否组合启动/)
  ...
  assert.match(english, /Before replacing the core, the new core composes every existing profile and its plugins/)

  assert.match(userReadme, /^DSH-Portable\r?\n=+/m)
  assert.match(userReadme, /只下载.+DSH.+组件/s)
  assert.match(userReadme, /替换内核前会先检查现有 Profile 与插件兼容性/)
...
test('installable official updates use a short-lived PR, full product gates, and an independent core channel', async () => {
  ...
  const { defaultEngineUpdateManifestUrl } = await import('../launcher/update-core.mjs')
  assert.match(defaultEngineUpdateManifestUrl('candidate', 'win32', 'x64', '0.6.5-rc.2'), /WSL043\/DSH-Portable-Updates\/releases\/download\/update-channel-core-candidate-0\.6\.5-rc\.2\//)
...
test('Windows package exposes real GUI executables with matching icon and an isolated portable launch', async () => {
  ...
  assert.match(source, /DSH_PROFILE_COMPATIBILITY_FAILED/)
  ...
  assert.match(source, /phase == "preflighting"/)
  ...
  assert.match(build, /PreviewAppSource[\s\S]+stage-local-integrations\.mjs/)
  ...
  assert.match(build, /shellSchema\s*=\s*25/)
  assert.match(build, /requiredShellSchema\s*=\s*25/)
...
test('macOS package is a movable signed app shell for both supported architectures', async () => {
  ...
  assert.match(build, /"shellSchema": 20/)
  assert.match(build, /"requiredShellSchema": 20/)
