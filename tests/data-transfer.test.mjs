...
for (const sessionFilename of ['session.jsonl.zstd', 'session.v2.jsonl.zstd', 'session.jsonl', 'session.v2.jsonl', 'session.v3.jsonl', 'session.v3.jsonl.zstd', 'session.v4.jsonl', 'session.v4.jsonl.zstd']) {
test(`restore relocates ${sessionFilename} without rewriting historical event text`, async () => {
  ...
  await writeFile(workspaceStore, `${JSON.stringify({ workspaces: {
    portable: { path: source.layout.workspace },
    external: { path: externalWorkspace },
  } })}\n`)
  ...
  const targetRoot = await mkdtemp(path.join(os.tmpdir(), 'dsh-data-transfer-relocated-'))
