...
test('Windows desktop host waits for the usable DOM and reports the failing boundary', async () => {
  ...
  assert.match(source, /UriBuilder probeUrl = new UriBuilder\(url\) \{ Query = String\.Empty, Fragment = String\.Empty \}/)
  assert.match(source, /host=401 auth-required \(service reachable\)/)
  assert.match(source, /RedactSensitiveText\(tail\)/)
