...
test('startup pauses only active third-party bundles whose declared host peers exclude the current runtime', async t => {
  ...
  for (const directory of [pluginRoot, currentRoot, hostRoot]) await mkdir(directory, { recursive: true })
  const manifestFile = path.join(profile, 'package.json')
  await writeFile(manifestFile, `${JSON.stringify({
    dependencies: { 'old-plugin': '1.0.0', 'current-plugin': '2.0.0' },
    dsh: { profile: { bundles: ['@deepseek-ai/dsh-web-app', 'old-plugin', 'current-plugin'] } },
  })}\n`)
  ...
  assert.deepEqual(updated.dsh.profile.bundles, ['@deepseek-ai/dsh-web-app', 'current-plugin'])
