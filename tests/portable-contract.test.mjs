...
test('moving the whole folder migrates only its owned workspace references', async () => {
  ...
  const compressedNames = ['session.jsonl.zstd', 'session.v2.jsonl.zstd', 'session.v3.jsonl.zstd', 'session.v4.jsonl.zstd']
  const plainNames = ['session.jsonl', 'session.v2.jsonl', 'session.v3.jsonl', 'session.v4.jsonl']
