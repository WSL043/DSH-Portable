...
test('bot dependency branches report qualification for a human merge', async () => {
  ...
  assert.match(stableIntake, /market-intake\/compare\.json/)
  assert.match(stableIntake, /name: plugin-market-review/)
  assert.match(stableIntake, /GITHUB_STEP_SUMMARY/)
  assert.doesNotMatch(stableIntake, /gh issue (?:create|edit|close)/)
})
