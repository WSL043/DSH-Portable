...
全新安装离线包含可停用或卸载的 [DSH Native Session Manager](https://github.com/WSL043/dsh-native-session-manager) 和 [DSH Native Image Viewer](https://github.com/WSL043/dsh-native-image-viewer)。前者提供归档管理和二次确认的永久删除会话，后者提供原生图片查看；两者之后都按普通插件更新。普通升级不会重新安装用户已经移除的默认插件。
