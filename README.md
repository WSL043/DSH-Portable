...
全新安装不会默认安装社区插件；可按需从插件市场或通过标准 DSH 命令安装。普通升级会完整保留现有 Profile 及其中已安装或已移除的插件，不会自动卸载、重装或改写。
...
- 更新通道可选**稳定版**或 **Beta 测试版**。稳定版适合日常使用；Beta 只接收已经通过 Portable 成品验证的预览版本，切换通道不会自动降级当前版本。
...
- 可以选择稍后或**跳过此版本**；安装前确认没有任务运行。替换内核前会先用新内核检查现有 Profile 和插件能否组合启动；不兼容时保持当前版本不变。更新后只有工作台真正就绪才会提交新版本，启动失败或超时会自动恢复更新前的程序，同时保留会话、设置、插件和工作区。
