...
**常规安装不需要打开终端或输入代码。** 搜索插件，打开详情，再点击安装；需要刷新或重启时按页面提示操作即可。从 Portable 0.7.3 起，市场默认安装正式版；只有 npm 的 Beta 标签指向更新的预发布版本时，才会单独显示安装或试用 Beta 的按钮。插件市场参考上游 [dsh-market](https://github.com/dsh-market/dsh-market)，安装操作交给 DSH 官方插件管理页面。
