...
下一阶段会把可靠退出、分层更新、依赖积累、跨平台一致性和可验证瘦身收敛到一个完整的 0.5.0，而不是把普通维护拆成连续小版本。具体边界和验收条件见 [0.5.0 路线图](ROADMAP.md)。
...
成品根目录的 `DATA-MIGRATION.txt` 提供检查和恢复命令。恢复默认只补入缺失数据；明确选择覆盖时，先在 `data/backups/` 生成回滚副本。插件市场中的「插件同步」只用于跨设备同步插件清单和插件配置，不是整机备份。
