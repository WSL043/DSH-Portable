...
When importing through Settings, the confirmation screen explains that package files will replace conflicts. Review the contents before choosing “Restart and import”; Portable creates a rollback copy under `data/backups/`. Command-line restore fills missing data by default and requires an explicit option to replace conflicts. If plugin dependency installation or profile validation fails, Portable rolls back that attempt, then imports usable sessions and settings while telling you that plugins and their own credentials were skipped. The original package remains available for a later retry. Registry plugins may need network access the first time they are restored on a new computer.
