...
**正式产品继续使用现有 Native 宿主；不启动 Electron 重写。** Portable 的产品边界是可搬迁的数据与运行环境、断网启动、独立且可回滚的内核更新，以及 Windows/macOS/Linux 交付。官方 Desktop 是另一种整套产品；将其 UI 直接换入现有宿主会同时改变数据、更新和签名的所有权，不能按普通界面升级处理。
...
3. **官方 Electron 只做独立验证。** 以官方 `0.1.7-rc.2` Nightly 发行物为第一个实物样本，核对下载摘要、签名、安装器与更新器行为；在专用空目录验证搬迁、离线冷启动、账号状态、插件、退出恢复和目录外写入。和同机 Native 成品实测首次可用时间、后续启动、内存及压缩/展开体积。不得接入现有用户 profile。官方 Windows Nightly feed 已列出该版本，稳定 feed 当日返回 404；Nightly 存在不等于正式发行或适合二次分发。
4. **只在证据成立时升级路线。** 如果官方原样发行物允许外部管理层掌握数据位置和唯一更新所有权，签名仍有效，且 Windows/macOS 的实机迁移、离线和故障恢复通过，再考虑提供独立的 Electron 试验版。Linux 仍由当前产品交付；缺少 Linux 官方桌面包时，不撤掉现有 Linux 宿主。若必须修改签名覆盖文件、与官方自更新争抢、或无法安全搬迁账号数据，就暂停此路线，继续吸收上游可复用的功能而非维护 Electron fork。

这四项有先后关系：Electron 研究不占用当前内核迁移和用户故障修复的发布门槛；上游已经提供的通用功能按成品验收逐项接管，验证后才删除 Portable 补丁。
