...
# Recovery boundary for plugin-origin startup failures

The independent Windows Recovery executable can repair generated module links, but that result does not prove the workspace or user plugins can load. A localhost probe returning HTTP 401 without the workspace token means the service answered and requires authentication; it is not evidence that WebView2 or the network is broken. Current desktop diagnostics label that state `host=401 auth-required (service reachable)` and include the Portable executable version.

Recovery now checks the active web profile bundle list against packages resolvable from the profile or packaged runtime. It reports a missing or invalid active bundle separately from damaged generated links. With Portable fully exited, the user can temporarily pause a selected community plugin or an official bundle absent from this runtime. This changes only `dsh.profile.bundles`, makes a private manifest backup, and records an undo entry; it does not uninstall packages or alter sessions, settings, or workspace. Installed official bundles and Portable's own bundles are protected. An absent official package must still be installed or supplied by a complete compatible runtime before its feature can be restored.

In the September 23 field logs, an older subscription plugin also failed activation with `ctx.settings.register is not a function`. That runtime API error and the missing experimental Agent Team profile are distinct findings. An isolated Recovery pause can establish whether the failing plugin blocks workspace startup; it is not a substitute for updating the plugin and restoring the missing dependency. The local copied logs identify Portable 0.7.0-beta.3 for an earlier attempt; they do not identify the later remote machine's current package.
