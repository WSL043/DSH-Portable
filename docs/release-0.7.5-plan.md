...
2026-09-27。`v0.7.5` 已正式发布；精确提交 `c82b979` 的成品 CI 与发布流程均通过。Electron `1.0.0-alpha.1` 是独立的新目录试用版，不作为本版升级验收。
...
| 历史会话 | 精确 alpha.1 已加入受摘要约束的只读迁移；旧 writer 夹具继续执行、保存重开及日志恢复通过 | 最终提交的跨平台成品与默认插件验收仍为发布条件 |
...
本实验是针对已发布包的确定性拒绝复现，不是真实历史会话全验收，也不说明全部普通会话不可用。详细本地输出：`build/native075-baseline/format-result.json`。这是修复前对照；后续执行清单记录迁移验收，`stable-release-readiness.json` 已绑定精确补丁身份，最终成品资格仍独立检查。
...
- [x] 最终提交 `c82b979f1a000013ef8837dd30c02b34da9ce3da` 的完整成品 CI `36265359810` 全部通过：五平台构建及操作、上一正式版升级、组件更新/回滚、两款默认插件逐项取消卸载/卸载/重装/启用、重启后输入可用；Windows 2022/2025 原生生命周期与亮暗操作通过。最新原生亮暗截图已人工检查，归档删除为红色、四个侧栏操作保留、图片标注与草稿回填正常。证据：`build/native075-final-c82b979/native-ui/result.json`、`plugin-cycle/result.json` 及对应 CI artifacts。CodeQL `36265359575` 成功。
...
原生官方插件页脚本原先只对图片插件执行取消卸载、卸载、重装及启用；现扩展为两款默认插件逐项执行，并分别记录实际安装版本和截图。隔离 0.7.4 对照包两款安装链路通过，但后续新增的重启确认按钮检查超时，整套结果仍为失败；须在最终 0.7.5 包重新执行，不将对照局部通过视作最终验收。

### 最终证据与旧截图清理

以 `c82b979` / CI `36265359810` 为本版最终成品证据。较早 `6d8833d` 成品的本机两款插件流程也通过，但不代替最终提交验收。按用户要求，67 张旧对照及较早提交截图已移入回收站，仅保留最新候选截图用于视觉判断；旧日志只用于故障溯源。发布流程 `36266467065` 使用最终成功 CI 的原始产物，发布流程已成功完成，GitHub latest 为 `v0.7.5`，stable Windows 更新清单确认 `portableVersion: 0.7.5`。

发布证据：[完整成品 CI](https://github.com/WSL043/DSH-Portable/actions/runs/36265359810)、[发布流程](https://github.com/WSL043/DSH-Portable/actions/runs/36266467065)、[正式下载](https://github.com/WSL043/DSH-Portable/releases/tag/v0.7.5)。Windows 标准 ZIP SHA-256：`8c39ccfa5dce19fa6c4ac34dd1a7ee8c27643673a6588fc26d447a79e2ac4d53`。本执行清单已完成；长期维护路线中的未完成研究仍保持未完成，不由本次发布代替。
