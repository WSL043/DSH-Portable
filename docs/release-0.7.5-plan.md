...
- [ ] 恢复的子会话实际继续执行、官方持久化保存后重新打开、中断恢复；完成后才评估产品补丁与资格摘要。
...
- [ ] 同次 CI Windows 2025 注入卡顿的 HTTP 时序断言失败；已补探测时间诊断，不放宽原断言。本机隔离原生复验通过，HTTP 21ms 完成，早于卡顿结束约 2.77s；日志 `build/native075-baseline/health-timing-local.log`。这不能解释 Windows 2025 原失败，仍须复用原 CI 产物定向取证。
