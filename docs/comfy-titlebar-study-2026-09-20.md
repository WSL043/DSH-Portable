...
状态只使用：待核查、待实现、源码完成、隔离验收通过、成品验收通过、已发布。只有相应证据存在才升级状态。下面均是待办，除明确列出的已有提交外不勾选完成。

| 编号 / 优先级 | 范围与下一步 | 完成条件 | 当前状态 / 依赖 |
| --- | --- | --- | --- |
| T01 / P0 | 在现有原生标题栏核查菜单开关、焦点、悬浮和提示生命周期 | 点击展开；再次点击关闭；外部点击、Esc、失焦关闭；连续操作不误重开、不残留高亮；不吞输入框首次点击 | 待核查；不得机械增加延时掩盖竞态 |
| T02 / P0 | 核查布局预算、菜单可用高度、边框与窗口命中测试 | 中英、亮暗、100/125/150/200% DPI、最小窗口、最大化还原及跨屏；按钮可达、菜单可滚到首尾、空白可拖动、四边四角可缩放 | 待核查；细边框与缩放旧待办统一归此，不重复建任务 |
| T03 / P0 | 核查原生命令与 WebView 快捷键共用分发 | 一次按键执行一次；禁用状态一致；输入法组合输入、编辑快捷键不被抢；重载后不多注册 | 待核查；已有 AddDesktopCommand，不重建平行命令表 |
| T04 / P1 | 制作本地顶部栏交互预览，验证信息密度与窄屏收纳 | 保留拖动空白、窗口操作及 Star；更新状态出现/消失无挤压；不加入官方会话标签的复制品；用户确认后再集成 | 待实现；依赖 T01–T03 的约束及 U01 状态协议 |
| U01 / P1 | 统一设置页、顶部栏和独立管理入口的更新状态 | Portable/内核身份独立；未知、检查中、无更新、有更新、下载、校验、待应用、健康检查、完成、失败、回滚清楚区分；重连先读快照，旧任务事件不能覆盖新任务 | 待实现；先盘点现有事件，避免新增轮询和重复下载 |
| U02 / P1 | 完善版本列表及自动内核目录同步反馈 | 当前/推荐/历史层次清楚；明确缺陷版本不可选；不兼容原因可读；查询失败不显示最新；升级后仍可查可兼容历史；桌面版本和内核版本不互相冒充 | 待核查；复用既有自动同步，回读真实发布索引，不以 workflow 启动成功结案 |
| U03 / P1 | 明确恢复范围与更新前后状态 | 提示是否重启及影响插件；任务进行中不擅自重启；失败保留可诊断原因并可恢复程序；不承诺程序回退等于数据格式可降级 | 待核查；沿用现有事务和健康检查，数据迁移单独验证 |
| U04 / P1 | 整理 Portable 管理页的信息架构 | 更新、存储与恢复、诊断入口职责清楚；插件仍用官方插件页；主 UI 启动失败时独立诊断可用；页面跳转或关闭不丢下载任务 | 待实现；先本地预览，沿用官方设计语言，不修改官方设置页主体 |
| S01 / P1 | 将 pnpm、旧运行时、下载临时文件与恢复材料纳入可解释存储视图 | 显示真实占用和清理范围；活动/被引用资源不删；离线修复仍可用；恢复材料明确保留上限；取消和失败可恢复 | 待核查；引用 maintenance-roadmap.md 的现有 pnpm 待办及 long-term-storage.md，禁止自动清空整个 store |
| Q01 / 发布门槛 | 用最终候选成品跑完整用户操作链 | 新装和旧版升级分别验收；输入可用；插件安装/更新/启停/卸载不误跳页面；归档恢复及删除取消；窗口操作；更新失败恢复；受影响平台离线验收 | 待执行；必须记录产品、插件、内核及验收脚本身份；已有旧提交成功不能顶替 |

### 状态与存储的硬边界

- UI 只投影任务状态；关闭管理页不取消后台任务，取消须经对应操作入口明确执行。
- 每次任务有唯一标识及阶段，过期事件丢弃；重复点击不会启动第二个相同安装事务。
- 首次连接回放历史任务不再次通知；错误仍可查阅，重开窗口不连续弹成功提示。
- 通知已读、任务历史和可再生文件均需有界；保留限额以测量和离线恢复验收确定，不先拍脑袋自动删除。
- 单个插件升级失败应说明该插件及原因；不能用其它插件成功掩盖失败，也不能静默改变用户禁用选择。
- 官方组件问题先归属核查。此前 `@` 面板裁切仅有源码推断，尚无纯官方复现；不写成已确认上游缺陷，也不擅自加入 Portable 补丁。

### 验收记录模板

每项记录：任务编号 → 修改前现象 → 提交/包校验和 → 操作与预期 → 实际结果 → 日志/截图路径 → 未覆盖项 → 是否发布。
自动化优先覆盖共用状态转换和完整操作链；真实缺陷补最小回归，不为每个按钮另写镜像实现测试。
失败保留现场，修复后复验失败链路；仅当变更影响范围需要时扩大测试，不为文档修改重复全量构建。
本轮清单更新不产生产品发布，也不表示上述工作已经完成。
