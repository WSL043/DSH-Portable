...
if (!code.startsWith(required)) {
  ...
  const compact = new RegExp(`^window\\.__ModuleLoader__\\.load\\(\\{id:[\\\`'\"]${quotedName}[\\\`'\"],factory:([A-Za-z_$][\\w$]*)=>\\{`)
  const compactMatch = code.match(compact)
  if (compactMatch !== null) {
    code = code.replace(compact, `window.__ModuleLoader__.load({ id: ${JSON.stringify(name)}, factory: (${compactMatch[1]}) => {`)
  } else {
