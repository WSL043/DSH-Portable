...
async function update(name: string, betaVersion?: string) {
  if (snapshot.busy || snapshot.checking || snapshot.done[name]) return
  publish({ busy: name, failures: { ...snapshot.failures, [name]: '' } })
  try {
    const response = await fetch('/dsh-market/update', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name, ...(betaVersion ? { betaVersion } : {}) }) })
    const body = await response.json()
...
export function PluginUpdateRow({ zh, name, busy = false, view }: { zh: boolean; name: string; busy?: boolean; view: 'summary' | 'action' }) {
  ...
  if (!status?.updateAvailable && !status?.betaAvailable && !done) return null
  ...
  if (view === 'action') {
    ...
    return <span className={css.actions}>
      ...
      {status?.betaAvailable && <Button variant="outline" size="sm" disabled={busy || state.checking || Boolean(state.busy)} onClick={() => void update(name, status.betaAvailable)}>{zh ? '试用 Beta' : 'Try Beta'}</Button>}
    </span>
  }
