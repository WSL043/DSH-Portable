...
function MarketLogo({ size = 16, style }: { size?: number; style?: CSSProperties; animated?: boolean }) {
...
export function MarketSection(props: MarketSectionProps) {
  ...
  const [marketView, setMarketViewState] = useState<'cards' | 'compact'>(savedMarketView)
  const setMarketView = (view: 'cards' | 'compact'): void => {
    setMarketViewState(view)
    try { localStorage.setItem('dshm-market-view', view) } catch { /* storage unavailable */ }
  }
              ...
              : (
                  <>
                    ...
                    {plugins.length === 0
                      ...
                      : (
                          <>
                            <div className={marketView === 'compact' ? `${css.grid} ${css.compactGrid}` : css.grid}>{pagePlugins.map(pluginCard)}</div>
