...
import { groupConflictsByOwner, isStaleUpdate, parseIgnoredBuilds, parsePrepareNotAllowed, RELEASE_AGE_OVERRIDE, retargetCollections, validateAddedPlugins, withHoistRecovery } from './install.js';
...
const SELF_NAMES = new Set(['@wsl043/dsh-portable-plugin-market', 'dshmarket', 'dsh-market']);
/**
                ...
                try {
                    await withMutationLock(response, 'install', async () => {
                        ...
                        const staleError = !stale
                            ? null
                            : staleReason === 'release-age'
                                ? '这个版本刚发布，包管理器默认暂缓安装。你可以稍后再试，或明确选择「立即更新」安装此版本。 / This version was newly published, so the package manager is waiting before installation. Try again later, or explicitly choose "Update now" to install this version.'
