...
function Section(props) {
    ...
    return (<section className={css.diagSection}>
...
export function Diagnostics(props) {
    ...
    return (<div className={css.diagPage}>
      <div className={css.diagSummary}>
        ...
        <span className={css.grow}/>
        {hasHardIssues && (<Button variant="outline" size="sm" onClick={startAgentFix} title={t('aiFixHint')}>
            {t('aiFix')}
          </Button>)}
      ...
      {fixFallback !== null && (<div className={css.fixFallback}>
          <p className={css.panelNote}>{t('aiFixFail')}</p>
          <textarea readOnly rows={10} className={css.fixFallbackText} value={fixFallback} onFocus={e => e.currentTarget.select()}/>
        </div>)}
      ...
      <Section title={t('checkErrors')} count={summary.errors.length} empty={t('checkErrorsEmpty')} overview={summary.errors.length > 0 ? summary.errors[0] : undefined}>
        <div className={css.diagList}>
          {summary.errors.map((line, i) => (<div key={i} className={css.err}>{line}</div>))}
        </div>
      </Section>
      ...
      <Section title={t('checkWarnings')} count={summary.warnings.length} empty={t('checkWarningsEmpty')} overview={summary.warnings.length > 0 ? summary.warnings[0] : undefined}>
        <div className={css.diagList}>
          {summary.warnings.map((line, i) => (<div key={i} className={css.warnLine}><span>{line}</span></div>))}
        </div>
      </Section>

      <Section title={t('checkBundles')} count={report.bundles.length} empty={t('checkBundlesEmpty')} problem={false} overview={<span>
        ...
        {report.bundles.map((bundle, i) => (<div key={bundle.name} className={css.diagBundle}>
            ...
            {bundle.directory !== null && (<div className={css.diagMeta}>
                <span className={css.diagKey}>{t('checkDir')}</span>
                <code className={css.spec}>{bundle.directory}</code>
              </div>)}
            {bundle.patchPath !== null && (<div className={css.diagMeta}>
      ...
      <Section title={t('checkDuplicates')} count={report.duplicates.length} empty={t('checkDuplicatesEmpty')} overview={report.duplicates.length > 0 ? `${report.duplicates[0]?.id} × ${report.duplicates[0]?.count}` : undefined}>
        <div className={css.diagList}>
          {report.duplicates.map(dup => (<div key={dup.id} className={css.diagRow}>
      ...
      <Section title={t('checkPeerMismatches')} count={peerConfirmed.length} empty={t('checkPeerEmpty')} overview={report.peerMismatches.length > 0
            ? t('checkPeerOverview')
    ...
    alwaysShowBody={peerInfo.length > 0}>
        {peerConfirmed.length === 0 ? (<div className={css.diagEmpty}>{t('checkPeerEmpty')}</div>) : (<div className={css.diagList}>
            {peerConfirmed.map((peer, i) => (<div key={i} className={css.diagRow}>
        ...
        {peerInfo.length > 0 && (<DisclosureRow icon={<IconChevronDownOutline14 size={14}/>} title={`${t('checkPeerInfo').replace('{0}', String(peerInfo.length))} (${peerInfo.length})`} expandable open={peerInfoOpen} onToggle={() => setPeerInfoOpen(o => !o)}>
            <div className={css.diagList}>
              {peerInfo.map((peer, i) => (<div key={i} className={css.diagRow}>
                  <code className={css.diagVal}>{peer.name}</code>
                  <span className={css.nm}>{peer.plugin}</span>
                  <span className={css.spec}>{t('checkRange')}: {peer.range}</span>
                  <span className={css.spec}>{t('checkResolved')}: {peer.resolved ?? '—'}</span>
      ...
      <Section title={t('checkMultiVersion')} count={report.multiVersion.length} empty={t('checkMultiEmpty')} overview={report.multiVersion.length > 0 ? `${report.multiVersion[0]?.name}: ${report.multiVersion[0]?.versions.join(' / ')}` : undefined}>
        <div className={css.diagList}>
          {report.multiVersion.map(mv => (<div key={mv.name} className={css.diagRow}>
      ...
      <Section title={t('checkOverrides')} count={report.overrides.length} empty={t('checkOverridesEmpty')} overview={report.overrides.length > 0 ? `${report.overrides[0]?.id} ← ${report.overrides[0]?.layer}` : undefined}>
        <div className={css.diagList}>
          {report.overrides.map((ov, i) => (<div key={i} className={css.ovRow}>
      ...
      <Section title={t('checkOrphans')} count={report.orphans.length} empty={t('checkOrphansEmpty')} overview={report.orphans.length > 0 ? `${report.orphans[0]?.id}（${t(orphanKindLabel(report.orphans[0]?.reason ?? ''))}）` : undefined}>
        <div className={css.diagList}>
          {report.orphans.map((orphan, i) => (<div key={i} className={css.orphRow}>
      ...
      <CollapsibleSection title={t('orderSection')} count={order.length} open={orderOpen} onToggle={() => setOrderOpen(o => !o)}>
        ...
        {report.orderConflicts !== undefined && report.orderConflicts.length > 0 && (<div className={css.diagList}>
            <span className={css.diagKey}>{t('orderConflicts')}</span>
            {report.orderConflicts.map((conflict, i) => (<div key={i} className={css.warnLine}>{conflict.name} — {conflict.reason}</div>))}
          </div>)}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          {order.join('\u0000') !== communityNames.join('\u0000') && (<Button variant="ghost" size="sm" disabled={orderBusy} onClick={() => setOrder(communityNames)}>
              {t('orderReset')}
            </Button>)}
          {orderMsg !== null && <span className={css.okState}>{orderMsg}</span>}
          {orderErr !== null && <span className={css.err}>{orderErr}</span>}
        </div>
        {orderDiff !== null && (<div className={css.panelNote}>
        ...
        {suggested !== null && suggested.ok === false && (<div className={css.warnLine}>{t('orderSuggestHint')} ⚠ {suggested.cycle.join(' → ')}</div>)}
        {report.duplicateNames !== undefined && report.duplicateNames.length > 0 && (<div className={css.diagList}>
            <span className={css.diagKey}>{t('duplicateNames')}</span>
            {report.duplicateNames.map((dup, i) => (<div key={i} className={css.panelNote}>{dup.name} × {dup.count} — {dup.layers.join(' / ')}</div>))}
          </div>)}
        {order.length === 0
            ? <div className={css.diagEmpty}>—</div>
            : (<div className={css.diagList}>
                {order.map((name, i) => (<div key={name} draggable={!orderBusy} className={[
                        css.diagRow,
                        dragIndex === i ? css.dragging : '',
                        dragOverIndex === i ? css.dragOver : '',
                    ].filter(Boolean).join(' ')} onDragStart={onRowDragStart(i)} onDragOver={onRowDragOver(i)} onDragLeave={onRowDragLeave(i)} onDrop={onRowDrop(i)} onDragEnd={onRowDragEnd}>
                    <span className={css.dragHandle} aria-label={t('orderDrag')} title={t('orderDrag')}>⠿</span>
                    <span className={css.diagIndex}>{i + 1}</span>
                    <span className={css.nm}>{name}</span>
                    <span className={css.grow}/>
                    <Button variant="ghost" size="sm" draggable={false} aria-label={t('orderUp')} disabled={i === 0 || orderBusy} onClick={() => moveBundle(i, -1)}>{t('orderUp')}</Button>
                    <Button variant="ghost" size="sm" draggable={false} aria-label={t('orderDown')} disabled={i >= order.length - 1 || orderBusy} onClick={() => moveBundle(i, 1)}>{t('orderDown')}</Button>
                  </div>))}
              </div>)}
      </CollapsibleSection>
    </div>);
