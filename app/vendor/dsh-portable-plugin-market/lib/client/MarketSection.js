...
import { avatarColor, entryForDep, groupSwitchState, humanOutput, isInstalled, matchInstalledName, orderedCategories, formatCount, pageItems, pluginName, pluginScreenshots, readSession, safeScreenshots, themePlugins as themePluginsOf, themeSwatch, TIME_RANGE_DAYS, visiblePlugins, } from './market-data.js';
...
function HostDependencyDiagnostics({ findings, t, }) {
    ...
    return (<div className={css.banner}>
      ...
      <span className={css.grow}>
        ...
        {preview.map(finding => (<div key={`${finding.subject.name}:${finding.evidence.dependency}`} className={css.spec}>
            {finding.subject.name} → {finding.evidence.dependency}@{finding.evidence.declaredRange}
          </div>))}
        {remaining > 0 && (<div className={css.spec}>{t('hostDependencyMore').replace('{0}', String(remaining))}</div>)}
      </span>
    </div>);
}
...
function OwnerAvatar({ name, owner }) {
    ...
    if (failed || owner === '') {
        return (<div className={css.av} style={{ background: avatarColor(name) }}>
        {name.replace(/^dsh[-_]/i, '').charAt(0).toUpperCase() || 'P'}
      </div>);
    }
    return (<img className={css.av} src={`https://github.com/${encodeURIComponent(owner)}.png?size=96`} alt="" loading="lazy" onError={() => setFailed(true)}/>);
}
/**
...
function ScreenshotStrip({ plugin, onOpen }) {
    ...
    if (visible.length === 0)
        return null;
    return (<div className={css.shots}>
      {visible.map((src, i) => (<img key={src} className={css.shot} src={src} alt="" loading="lazy" decoding="async" referrerPolicy="no-referrer" onClick={() => onOpen(visible, i)} onError={() => setBroken(prev => prev.includes(src) ? prev : prev.concat(src))}/>))}
    </div>);
}
/**
...
function CardShot({ plugin, onOpen }) {
    ...
    if (visible.length === 0)
        return null;
    return (<div className={css.cardShots}>
      {visible.map((src, i) => (<img key={src} className={css.cardShot} src={src} alt="" loading="lazy" decoding="async" fetchPriority="low" referrerPolicy="no-referrer" onClick={(e) => { e.stopPropagation(); onOpen(visible, i); }} onError={() => setBroken(prev => prev.includes(src) ? prev : prev.concat(src))}/>))}
    </div>);
}
/**
...
function ScreenshotLightbox({ shots, startIndex, onClose, t }) {
    ...
    return createPortal(<div className={css.lightbox} onClick={onClose}>
      ...
      {shots.length > 1 && (<>
          <button className={`${css.lightboxNav} ${css.lightboxPrev}`} aria-label={t('lightboxPrev')} onClick={(e) => { e.stopPropagation(); setIndex(index - 1); }}><IconChevronLeftOutline14 size={18}/></button>
          <button className={`${css.lightboxNav} ${css.lightboxNext}`} aria-label={t('lightboxNext')} onClick={(e) => { e.stopPropagation(); setIndex(index + 1); }}><IconChevronRightOutline14 size={18}/></button>
          <div className={css.lightboxDots} onClick={e => e.stopPropagation()}>
            {shots.map((src, i) => (<span key={src} className={i === index ? `${css.lightboxDot} ${css.lightboxDotOn}` : css.lightboxDot} onClick={() => setIndex(i)}/>))}
          </div>
        </>)}
    </div>, document.body);
}
...
function MarketLogo({ size = 16, style }) {
    return <span style={{ ...style, display: 'inline-flex' }}><FishLogo size={size}/></span>;
}
/**
...
export function MarketSection(props) {
    ...
    const filterSelectedIds = useMemo(() => ['field:' + sortField, 'dir:' + sortDir, 'time:' + timeRange], [sortField, sortDir, timeRange]);
    const onFilterSelect = (id) => {
        if (id.startsWith('field:'))
            setSortField(id.slice(6));
        else if (id.startsWith('dir:'))
            setSortDir(id.slice(4));
        else if (id.startsWith('time:'))
            setTimeRange(id.slice(5));
    };
    ...
    const pluginCard = (p) => {
        ...
        return (<div key={p.url} className={blocked ? `${css.card} ${css.cardBlocked}` : css.card}>
                    ...
                    : busy
                        ...
                        : blocked
                          ...
                          {t('opBlockedCard')}
                            ...
                            : (<Button variant="primary" size="sm" className={css.installBtn} disabled={busyUrl !== null || !envReady} onClick={() => setConfirming(p)}>{t('install')}</Button>)}
          </div>
        </div>
        <div className={css.desc}>{desc}</div>
        <CardShot plugin={p} onOpen={openLightbox}/>
        {p.deprecated === true && (<div className={css.deprecate}>
            <div className={css.depLine}>
              <span>⚠️ {t('deprecatedWarn')}</span>
              {replacement !== undefined && (<a className={css.src} href={replacement.url} target="_blank" rel="noreferrer">
        ...
        {busy && (<div className={css.progress}>
            ...
            <div className={css.bar}>
              <div className={progressPct !== null ? css.barFill : `${css.barFill} ${css.barWave}`} style={progressPct !== null ? { width: `${progressPct}%` } : undefined}/>
            </div>
          </div>)}
      </div>);
    };
    ...
    const themePluginCard = (p) => {
        ...
        const desc = (p.description && (p.description[lang] || p.description.en)) || '';
        const replacement = replacementOf(p);
        return (<div key={p.url} className={css.card}>
        <div className={css.row1}>
        ...
        <div className={css.foot}>
          ...
          {mounted
                ...
                <Button variant="outline" size="sm" disabled={togglingName !== null} onClick={() => doToggle(instName, false, true)}>{t('themeDeactivate')}</Button>
              </>
    ...
    const themeCard = (id, label, swatch) => {
        ...
        return (<div key={'th-' + id} className={css.card}>
        ...
        <div className={css.foot}>
          ...
          {active
                ? <span className={css.okState}>{t('themeActive')}</span>
                : (<Button variant="primary" size="sm" onClick={() => { try {
    ...
    return (<div className={css.root}>
      <div className={css.tabs}>
          ...
          <button className={(tab === 'backup' || tab === 'diagnostics') ? `${css.tab} ${css.on}` : css.tab} onClick={() => { if (tab !== 'backup' && tab !== 'diagnostics')
        setTab('backup'); }}>{t('tabAdvanced')}</button>
          <span className={css.grow}/>
          {updatableNames.length >= 2 && (<Button variant="primary" size="sm" disabled={updatingAll || updatingName !== null || busyUrl !== null || removingName !== null} onClick={() => { setTab('installed'); doUpdateAll(); }}>{updatingAll ? t('updating') : t('updateAll') + ' (' + updatableNames.length + ')'}</Button>)}
          {/* In the tab row, not above the grid: paginating, searching and
            switching tab all leave it — and any pending decision — in place. */}
          <OperationsPanel t={t} describe={describePlugin} records={records} open={operationsOpen} onOpenChange={setOperationsOpen} replacing={replacing} envReady={envReady} onClearSettled={() => setRecords(list => clearSettled(list))} onCancel={() => doCancel()} onDismiss={record => setRecords(list => drop(list, record.id))} onRefresh={() => location.reload()} onResolveConflict={resolveConflict}/>
      </div>
      <div className={css.notices}>
        ...
        {(tab === 'backup' || tab === 'diagnostics') && (<div className={css.subTabs}>
        ...
        {!envReady && (<div className={css.banner}>
            ...
            {!envFailed && (<Button variant="primary" size="sm" disabled={envFixing} onClick={fixEnv}>
                {envFixing ? t('envFixing') : t('envFix')}
              </Button>)}
          </div>)}
        {backupMessage !== null && <div className={css.backupMessage}>{backupMessage}</div>}
        {restoreErrors.length > 0 && (<div className={css.banner}>
            <IconWarningOutline16 size={14} className={css.bannerIcon}/>
            <span className={css.grow}>
        ...
        {tab === 'installed' && pendingBackup !== null && (<div className={css.banner}>
        ...
        {pendingRefreshNames.length > 0 && (<div className={css.banner}>
        ...
        {pendingRestart > 0 && (<div className={css.banner}>
            <IconRefreshOutline14 size={14} className={css.bannerIcon}/>
            ...
            {restartEnabled && (<Button variant="primary" size="sm" disabled={restarting || hostBusy || busyUrl !== null || updatingName !== null || removingName !== null} onClick={doRestart}>{restarting ? t('restarting') : t('restartNow')}</Button>)}
            ...
            {showHostPending && (<Button variant="ghost" size="sm" aria-label={t('dismissNotice')} onClick={() => {
        ...
        {activationWarnings.length > 0 && (<div className={css.banner}>
            <IconWarningOutline16 size={14} className={css.bannerIcon}/>
            <span className={css.grow}>
              {activationWarnings.map(({ name, info }) => (<div key={name}>
      ...
      {buildsSkipped !== null && (<div className={css.banner}>
          <IconWarningOutline16 size={14} className={css.bannerIcon}/>
          <span className={css.grow}>{t('buildsSkipped')} {buildsSkipped.names.join(', ')}</span>
          <Button size="sm" disabled={busyUrl !== null} onClick={() => {
      ...
      {compatibilityNotice !== null && (<div className={css.banner}>
          <span className={css.grow}>
      ...
      {installError !== null && (<div className={css.err}>
          {installError}
          <div className={css.staleAction}>
            {staleName !== null && (<Button size="sm" onClick={() => doUpdate(staleName, true)}>{t('updateNow')}</Button>)}
          </div>
        </div>)}
      <div className={css.body} ref={bodyRef} onScroll={e => setShowTop(e.currentTarget.scrollTop > 400)}>
        {tab === 'backup'
            ? (<div className={css.backupGrid}>
                <section className={css.backupCard}>
                  ...
                  <div className={css.backupActions}>
                    <Button variant="primary" size="sm" icon={<IconDownloadOutline16 size={14}/>} disabled={backupBusy} onClick={() => downloadFile('/dsh-market/backup', 'dsh-profile-backup.json')}>{backupBusy ? t('backupWorking') : t('backupDownload')}</Button>
                    <Button variant="outline" size="sm" icon={<IconFolderOpen16 size={14}/>} disabled={backupBusy} onClick={() => fileInputRef.current?.click()}>{backupBusy ? t('backupWorking') : t('backupImport')}</Button>
                    <input ref={fileInputRef} type="file" accept="application/json,.json" className={css.hiddenFile} tabIndex={-1} aria-hidden="true" disabled={backupBusy} onChange={event => {
                ...
                <section className={css.backupCard}>
                  <h3>{t('webdav')}</h3>
                  <Menu open={presetOpen} onClose={() => setPresetOpen(false)} onSelect={id => {
                    const urls = {
                ...
                }} align="start" anchor={(<Button variant="outline" size="sm" icon={<IconChevronDownOutline14 size={14}/>} onClick={() => setPresetOpen(o => !o)}>{t('webdavPreset')}</Button>)} items={[
                ...
                <section className={css.backupCard}>
                  <h3>{t('gist')}</h3>
                  <Input className={css.backupInput} type="password" autoComplete="off" value={gistToken} placeholder={t('gistToken')} onChange={e => setGistToken(e.target.value)}/>
                  <Input className={css.backupInput} icon={<IconLinkOutline14 size={14}/>} value={gistId} placeholder={t('gistId')} onChange={e => setGistId(e.target.value)}/>
                  <div className={css.backupActions}>
                  ...
                  {gistResult !== null && (<p className={css.backupCheck}>
                      <span>{t('gistCreated')}</span>{' '}
                      <a className={css.src} href={gistResult.gistUrl} target="_blank" rel="noreferrer">{gistResult.gistUrl}</a>
                    </p>)}
                  {gistMessage !== null && (<div className={gistOk ? css.backupMessage : css.backupWarn}>{gistMessage}</div>)}
                  <p>{t('gistNote')}</p>
                </section>
              </div>)
            : tab === 'discover'
                    ...
                    <div className={css.cats}>
                      ...
                      <div ref={catsWrapRef} className={visibleCats === null ? `${css.catsWrap} ${css.catsCollapsed}` : css.catsWrap}>
                        {(() => {
                              ...
                              {shown.map(id => (<Pill key={id} data-chip="1" active={cat === id} onClick={() => setCat(id)}>{(data.categories[id] && (data.categories[id][lang] || data.categories[id].en)) || id}</Pill>))}
                              <Button variant="ghost" size="sm" className={css.catsToggle} icon={catsOpen ? <IconChevronUpOutline14 size={14}/> : <IconChevronDownOutline14 size={14}/>} aria-label={catsOpen ? t('catsLess') : t('catsMore')} onClick={() => {
                      ...
                      <Menu open={filterOpen} onClose={() => setFilterOpen(false)} onSelect={onFilterSelect} selectedIds={filterSelectedIds} align="end" portal anchor={(<Button variant="outline" size="sm" icon={filterOpen ? <IconChevronUpOutline14 size={14}/> : <IconChevronDownOutline14 size={14}/>} onClick={() => setFilterOpen(o => !o)}>{t('filter')}</Button>)} items={filterItems}/>
                      </div>
                    </div>
                    </div>
                    {plugins.length === 0
                            ...
                            <div className={css.pager}>
                              <div className={css.pagerPages}>
                                {totalPages > 1 && (<>
                                    ...
                                    <Button variant="outline" size="sm" icon={<IconChevronLeftOutline14 size={14}/>} disabled={currentPage === 1} onClick={() => goToPage(currentPage - 1)}>{t('prevPage')}</Button>
                                    {pageItems(currentPage, totalPages).map((item, i) => (item === '…'
                                            ? <span key={'e' + i} className={css.pageEllipsis}>…</span>
                                            : (<Button key={item} variant={item === currentPage ? 'primary' : 'outline'} size="sm" onClick={() => goToPage(item)}>{item}</Button>)))}
                                    <Button variant="outline" size="sm" disabled={currentPage === totalPages} onClick={() => goToPage(currentPage + 1)}>{t('nextPage')}<IconChevronRightOutline14 size={14}/></Button>
                              ...
                              <Menu open={sizeOpen} onClose={() => setSizeOpen(false)} onSelect={id => changePageSize(Number(id))} selectedId={String(pageSize)} align="end" portal anchor={(<Button variant="outline" size="sm" icon={<IconChevronDownOutline14 size={14}/>} onClick={() => setSizeOpen(o => !o)}>{t('perPage') + ' ' + pageSize}</Button>)} items={PAGE_SIZES.map(size => ({ id: String(size), label: String(size) }))}/>
                            </div>
                          </>)}
                  </>)
                : tab === 'themes' && themeSnap !== null
                  ...
                  {(() => {
                            ...
                            return extra.length > 0 && (<div className={`${css.grid} ${css.themesGrid}`}>
                  ...
                  {installedView === 'groups'
                            ...
                            <div className={css.groupCreate}>
                              {creatingGroup
                                        ? (<>
                                      <Input className={css.inlineInput} placeholder={t('groupNamePh')} value={newGroupName} onChange={e => setNewGroupName(e.target.value)} onKeyDown={e => { if (e.key === 'Enter')
                                            doCreateGroup(); }} autoFocus/>
                            ...
                            {groupOrder.length === 0
                                        ...
                                        : groupOrder.map(gid => {
                                            ...
                                            return (<div className={css.groupRow} key={gid}>
                                      <div className={css.groupHead}>
                                        <button type="button" role="switch" aria-checked={sw === 'on' ? true : sw === 'off' ? false : 'mixed'} aria-label={(sw !== 'on' ? t('enable') : t('disable')) + ' ' + gid} className={sw === 'on' ? `${css.switch} ${css.switchOn}` : sw === 'mixed' ? `${css.switch} ${css.switchMixed}` : css.switch} disabled={togglingName !== null || sw === 'empty'} onClick={() => doGroupToggle(gid, sw !== 'on')}>
                                        ...
                                        <div className={css.groupActions}>
                                          {renamingGroup === gid
                                                    ? (<>
                                                  <Input className={css.inlineInput} placeholder={t('groupNamePh')} value={renamingValue} onChange={e => setRenamingValue(e.target.value)} onKeyDown={e => { if (e.key === 'Enter')
                                                        doRenameGroup(gid); }} autoFocus/>
                                          ...
                                          <Button variant="outline" size="sm" onClick={() => setAddPanel(addPanel !== null && addPanel.group === gid && addPanel.kind === 'plugin'
                                                    ? null
                                                    : { group: gid, kind: 'plugin' })}>{t('groupAdd')}</Button>
                                          <Button variant="outline" size="sm" disabled={members.some(member => installedThemeNames.has(member))} onClick={() => setAddPanel(addPanel !== null && addPanel.group === gid && addPanel.kind === 'theme'
                                                    ? null
                                                    : { group: gid, kind: 'theme' })}>{t('groupAddTheme')}</Button>
                                        </div>
                                      </div>
                                      {addPanel !== null && addPanel.group === gid && (() => {
                                                    ...
                                                    return (<div className={css.groupAddPanel}>
                                            {candidates.length === 0
                                                            ? <div className={css.groupHint}>{t('groupAddEmpty')}</div>
                                                            : candidates.map(name => (<div className={css.groupMember} key={name}>
                                      ...
                                      <div className={css.groupMembers}>
                                        ...
                                        {members.map(member => (<div className={css.groupMember} key={member}>
                            ...
                            {ungroupedNames.length === 0
                                        ...
                                        : ungroupedNames.map(name => {
                                            ...
                                            return (<div className={css.irow} key={'ug-' + name}>
                                      <div style={{ minWidth: 0 }}>
                          ...
                          {Object.entries(displayedInstalled)
                                            ...
                                            return (<div key={name} className={missing ? `${css.irow} ${css.irowMissing}` : css.irow}>
                                <div style={{ minWidth: 0 }}>
                                  <div className={css.nm}>
                                  ...
                                  {entry !== undefined && (<div className={`${css.desc} ${css.descTight}`}>
                                      {(entry.description && (entry.description[lang] || entry.description.en)) || ''}
                                    </div>)}
                                  {!off && act !== undefined && meta !== null && (<div className={css.act}>
                                          ...
                                          {meta.dot !== 'done' && (<span className={meta.dot === 'error' ? css.actBroken : css.actWarn}>
                                              <StateDot state={meta.dot} size={7}/>
                                              {meta.label}
                                            </span>)}
                                          {act.state !== 'live' && act.reasons.length > 0 && (<DisclosureRow icon={<IconQuestionOutline14 size={14}/>} title={t('actWhy')} open={whyOpen === name} expandable expandOnRowClick onToggle={() => setWhyOpen(whyOpen === name ? null : name)} className={css.actWhy}>
                                              <div className={css.spec}>{act.reasons.join(' / ')}</div>
                                            </DisclosureRow>)}
                                        </div>)}
                                  {entry !== undefined && entry.deprecated === true && (<div className={css.deprecate} style={{ marginTop: 8 }}>
                                      <div className={css.depLine}>
                                        <span>⚠️ {t('deprecatedWarn')}</span>
                                        {entry.replacement !== undefined && (<span className={css.src}>{t('replacementHint') + ' ' + entry.replacement}</span>)}
                                      </div>
                                    </div>)}
                                  {updatingName === name && (<div className={css.progress}>
                                      <span className={css.spin}><IconLoadingOutline16 size={14}/></span>
                                      <code className={css.grow}>{progressText}</code>
                                      {progressPct !== null && <span className={css.pct}>{progressPct}%</span>}
                                      <Button variant="outline" size="sm" disabled={cancelling} onClick={doCancel}>
                                ...
                                {!missing && (<span className={css.stateTag} data-on={off ? 'false' : 'true'}>
                                    <span className={css.stateDot} data-on={off ? 'false' : 'true'}/>
                                    {off ? t('disabledState') : t('switchOnLabel')}
                                  </span>)}
                                {toggleable && (<button type="button" role="switch" aria-checked={!off} aria-label={(off ? t('enable') : t('disable')) + ' ' + name} className={off ? css.switch : `${css.switch} ${css.switchOn}`} disabled={togglingName !== null || busyUrl !== null || updatingName !== null || removingName !== null} onClick={() => doToggle(name, off)}>
                                    <span className={css.switchKnob}/>
                                  </button>)}
                                ...
                                {entry !== undefined && entry.deprecated === true && entry.replacement !== undefined && (() => {
                                      ...
                                      {!isInstalled(replacement, installed, repoIdentities, data?.plugins, repoHints) && (<Button variant="outline" size="sm" onClick={() => setConfirming(replacement)}>{t('installReplacement')}</Button>)}
                                    </>);
                                                })()}
                                {missing
                                                    ...
                                                    : updatedNames.includes(name)
                                                        ...
                                                        : updatingName === name
                                                            ...
                                                            : status && status.updateAvailable
                                                                ? (<Button variant="primary" size="sm" className={css.warnBtn} disabled={updatingName !== null} onClick={() => doUpdate(name)}>{t('update')}</Button>)
                                                                : status && status.kind === 'linked'
                                                                    ? <span className={css.metaTag}>{t('linkedDev')}</span>
                                                                    : <span className={css.metaTag}>{t('upToDate')}</span>}
                                {!missing && name !== 'dsh-market' && name !== 'dshmarket' && (removingName === name
                                                    ? <Button variant="outline" size="sm" className={css.dangerBtn} disabled>{t('uninstalling')}</Button>
                                                    : (<Button variant="outline" size="sm" className={css.dangerBtn} disabled={removingName !== null || busyUrl !== null || updatingName !== null} onClick={() => setRemoveConfirm(name)}>{t('uninstall')}</Button>))}
                                </div>
                              </div>);
                                        })}
                          </div>)}
      ...
      {showTop && (<Tooltip label={t('backTop')} side="top">
          <span className={css.top}>
            <Button variant="outline" className={css.topBtn} aria-label={t('backTop')} onClick={() => { const el = bodyRef.current; if (el)
            el.scrollTo({ top: 0, behavior: 'smooth' }); }}><IconChevronUpOutline14 size={16}/></Button>
          </span>
        </Tooltip>)}
      {confirming !== null && (<Modal open onClose={() => { setConfirming(null); setCmdOpen(false); }} title={t('confirmTitle') + ' ' + confirming.name + '?'} description={(confirming.description && (confirming.description[lang] || confirming.description.en)) || ''} footer={(<>
          ...
          <DisclosureRow icon={<IconCodeOutline16 size={16}/>} title={t('cmdDetails')} open={cmdOpen} expandable expandOnRowClick onToggle={() => setCmdOpen(o => !o)}>
            <div className={css.cmd}>{confirming.install}</div>
          </DisclosureRow>
          {confirming.deprecated === true && (() => {
                const replacement = replacementOf(confirming);
                return (<div className={css.deprecate}>
                <div className={css.depLine}>
                  <span>⚠️ {t('deprecatedWarn')}</span>
                  {replacement !== undefined && (<a className={css.src} href={replacement.url} target="_blank" rel="noreferrer">
                      {t('replacementHint') + ' ' + replacement.name}
      ...
      {lightbox !== null && (<ScreenshotLightbox shots={lightbox.shots} startIndex={lightbox.index} onClose={() => setLightbox(null)} t={t}/>)}
      {removeConfirm !== null && (<Modal open onClose={() => setRemoveConfirm(null)} title={t('uninstall') + ' ' + removeConfirm + '?'} description={t('uninstallConfirmDesc')} footer={(<>
      ...
      {restoreConfirmOpen && pendingBackup !== null && (<Modal open onClose={() => setRestoreConfirmOpen(false)} title={t('restoreConfirm')} footer={(<>
      ...
      {exportOpen && (<Modal open onClose={() => setExportOpen(false)} title={t('gistExportSelect')} description={t('gistExportHint')} footer={(<>
          ...
          {exportOptions.length > 0 && (<>
              ...
              <div className={css.backupCheckList}>
                {exportOptions.map(name => (<label key={name} className={css.backupCheck}>
                    <input type="checkbox" checked={exportSelection.has(name)} onChange={e => {
